...
## 기술 스택
- **엔진:** Unity (C#) / 2D URP — 최종 빌드 대상
- **프로토타입:** Love2D (Lua) — 현재 플레이 가능한 프로토타입
- **타겟:** Steam (PC)
...
## Love2D 프로토타입 (현재 플레이 가능)

`dokkaebi-love2d/` 디렉토리에 플레이 가능한 프로토타입이 있습니다.
...
### 구현된 기능
- 전투: 고스톱 매칭 → 족보 등록(시너지) → 섯다 공격 → 보스 HP 차감
- 3종 족보 체계: 고스톱 / 섯다 / 저승 (카테고리별 그룹 표시)
- 보스: 10종 랜덤 보스 + 기믹 + 격노 페이즈
- 부적: 20종 (TalismanDatabase 연동, 중복 방지)
- 상점: 부적 DB 직접 연동, 보유 부적 자동 제외
- 이벤트: 5종 (선택이 다음 보스전에 영향 — HP 변조, 기믹 봉인 등)
- BGM: 5트랙 CC0 + 상태별 자동 전환 + 크로스페이드
- SFX: 24종 프로시저럴 효과음 (외부 파일 없음)
- UI: 손패 정렬, 픽셀아트 설정 아이콘, 1% 볼륨 슬라이더, 중앙 연출 메시지
- 축복 4종, 강화, 영구 강화 트리, 도감(업적/부적/족보)

## 빌드
- **Love2D:** [love2d.org](https://love2d.org) 설치 후 `love dokkaebi-love2d`
- **Unity:** Unity 2022.3 LTS 이상에서 프로젝트를 열어주세요
